...
fun UploadDefaultExample() {
    val files = remember { mutableStateListOf(DarwinUploadFile(name = "document.pdf", url = "https://example.com/doc.pdf", progress = 1f), DarwinUploadFile(name = "image.png", isUploading = true, progress = 0.6f)) }
    DarwinUpload(files = files, onPickFiles = { files.add(DarwinUploadFile(name = "new-file.txt", isUploading = true, progress = 0.2f)) }, onRemoveFile = { index -> files.removeAt(index) }, modifier = Modifier.fillMaxWidth(0.5f))
}

@GalleryExample("Upload", "Compact")
@Composable
fun UploadCompactExample() {
    DarwinUpload(files = emptyList(), onPickFiles = {}, variant = DarwinUploadVariant.Compact, modifier = Modifier.fillMaxWidth(0.5f))
}

@GalleryExample("Upload", "Inline")
@Composable
fun UploadInlineExample() {
    DarwinUpload(files = emptyList(), onPickFiles = {}, variant = DarwinUploadVariant.Inline)
}
...
private fun UploadPage() {
    GalleryPage("Upload", "A file upload component with drag and drop support.") {
        SectionHeader("Examples")
        ExampleCard(title = "Default", sourceCode = GallerySources.UploadDefaultExample) { UploadDefaultExample() }
        ExampleCard(title = "Compact", sourceCode = GallerySources.UploadCompactExample) { UploadCompactExample() }
        ExampleCard(title = "Inline", sourceCode = GallerySources.UploadInlineExample) { UploadInlineExample() }
    }
}
